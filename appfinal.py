...
from pages import page1, page2, navigate, page3, page4, page5
...
page_tittle = navigate().title
if page_tittle == "Welcome Page":
    page3()
if page_tittle == "Admin":
    uploaded_file = page1()
    ...
    if uploaded_file:
        ...
        st.write("Using existing data in vector store.")


# Right side for bug description input
# st.title("Bug Resolution Predictor")
if page_tittle == "Developer" or page_tittle == "Admin":
    description = page2()
    bug_description = description[0]
...
if page_tittle == "Issuer":
    description = page4()
    new_bug_description = description[0]

    # Debugging statement
    if not isinstance(new_bug_description, str):
        st.error("Bug description should be a string.")           

if page_tittle == "Issuer":
    submit_button = description[1]
    if submit_button:
        if new_bug_description:
            try:
                # Generate embedding for new bug description
                new_bug_embedding = get_embedding(new_bug_description)
                if isinstance(new_bug_embedding, np.ndarray):
                    new_bug_embedding = new_bug_embedding.tolist()
                ...
                if search_results:
                    ...
                    if n_bugs > 0:
                        st.write(f"Thanks for raising the bug based on previous history {n_bugs} Similar Bugs Found")
                        resolution_times = []
                        bug_data = []
                        for result, score in filtered_results:
                            try:
                        ...
                        st.write("Thanks for raising the bug, no similar bugs found in our previous history, it will be added to our data to assist you next time.")
                    
            except Exception as e:
                st.error(f"Error during bug analysis: {str(e)}")

if page_tittle == "Data Dashboard":
    page5()